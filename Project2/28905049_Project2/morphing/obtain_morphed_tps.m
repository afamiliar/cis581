...
% pad if necessary
[rows_im, cols_im, ~] = size(im_source);
if rows == rows_im && cols == cols_im
    im_padded = im_source;
else
    im_padded = padarray(im_source, [rows-rows_im, cols-cols_im], 'replicate', 'post');
end
...
%   intermediate image
for i = 1 : rows*cols
    morphed_im(pixInd(2,i), pixInd(1,i), :) = im_padded(pixPos(2, i), pixPos(1, i), :);
end
